...
const getIntersections = (wiresInput) => {
    ...
    for(let i = 0; i < wire2Commands.length; i++) {
        ...
        for(let x = 0; x < command.amount; x++) {
            ...
            if (intersectingCoord) {
                intersections.push({ x: cursorX, y: cursorY, steps: intersectingCoord.steps + stepCounter });
            }
        }
    }
...
const getNearestIntersectionDistance = (wiresInput) => {
    const intersections = getIntersections(wiresInput);
